...
class Client:
    ...
    def __init__(
        ...
        *,
        ...
        flush_interval: float = 0.0,   # set > 0 to enable background flushing
    ) -> None:
    ...
    def log_metric(self, *, method: str, metric: str, value: float, step: int | None = None, run_id: str | None = None) -> None:
        ...
        # Note: step is accepted for API compatibility but intentionally not stored
        # as a metric series — it would produce a spurious "STEP" chart in the UI.
        # The x-axis is derived from list index server-side.
        with self._lock:
    ...
    def flush(self, *, run_id: str | None = None) -> None:
        ...
        if not rid:
            return
        # Snapshot the full cumulative buffer but do NOT clear it.
        # "series" on the server replaces the stored list, so we must always
        # send the complete history, not just what accumulated since last flush.
        # Clearing here would cause each flush to contain only the last interval,
        # leaving the chart showing just 1-2 points perpetually.
        with self._lock:
        ...
        *,
        ...
        if path is not None:
            ...
            try:
                ...
                if r.status_code >= 400:
                    raise RequestFailedError(f"POST images failed: {r.status_code} {r.text}", r.status_code)
            except requests.RequestException as e:
